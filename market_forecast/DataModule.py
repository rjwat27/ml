...
class DataBase():
    ...
    def Load(self, fname=None, file_dir=None):     #raw data
        # if fname is None:
        #     fname = self.filename
        '''TODO select files based on security type, for now all bitcoin'''
        local_dir = os.path.dirname(__file__)
        file_dir = local_dir+"/tick data" if not file_dir else file_dir#os.path.join(local_dir, self.filename)
        # self.size = 0 
    ...
    def list_of_samples(self, num_of_samples=100, len_of_samples=50, tick_epoch=0):
        ...
        for d in data:
            ...
            result = self.raw_strip(d, desired_keys) 
            training_prices.append(price) 
